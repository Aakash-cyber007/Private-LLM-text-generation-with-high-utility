'''Code to generate synthetic data using the method of Amin et al. (2024)'''
...
if __name__=='__main__':
    ...
    parser.add_argument('--sess', default='Amin et al. (2024)', type=str, help='session name')
    parser.add_argument('--method', default='amin', type=str, help='method name')
    ...
    parser.add_argument('--eps', default=40, type=float, help='privacy budget (epsilon)')
    ...
    parser.add_argument('--batch', default=16, type=int, help='batch size for generation')
    ...
    clip = utils.get_clip(args.eps, args.ptokens, args.temp, args.batch, args.sigma)
    if clip == np.inf: raise ValueError('Clipping norm must be finite! (Recommended: Increase epsilon or batch size)')
        
    # get folder and expt names [results/dataset/method]
    expt_name = args.model + '-eps_{}-batch_{}-tokens_{}-ptokens_{}-clip_{:.1f}-temp_{:.1f}-sigma_{}-theta_{}'
    expt_folder = os.path.join(os.path.join(args.folder, args.dataset), args.method)
    expt_name = expt_name.format(args.eps, args.batch, args.tokens, args.ptokens, clip, args.temp, args.sigma, args.theta)
    ...
    for i in tqdm(range(args.num)):
        ...
        counter = 0
        theta_hat = args.theta + np.random.laplace(0, args.sigma)
        for _ in range(args.tokens):
            ...
            avg_logits = np.mean(prv_logits, axis=0)
            clipped_logits = utils.clip_amin(prv_logits, clip)
            avg_clip_logits = np.mean(clipped_logits, axis=0)
            
            # distance calculation
            d = np.linalg.norm(spl.softmax(avg_clip_logits) - spl.softmax(pub_logits), ord=1)
            d_hat = d + np.random.laplace(0, 2*args.sigma)
            probs = None

            # SVT
            if d_hat > theta_hat:
                theta_hat = args.theta + np.random.laplace(0, args.sigma)
                probs = spl.softmax(avg_clip_logits/args.temp)            # release private data
                counter += 1
            else: probs = spl.softmax(pub_logits/args.temp_pub)               # release public data
            ...
            # break loop if EOS is encountered or if private token count is exceeded
            if counter >= args.ptokens: break
        ...
        results['len'].append(len(results['token_seq'][-1]))
        results['prv_len'].append(counter)
        
        # get privacy guarantees
        rho_calc = utils.compute_rho(counter, clip, args.batch-1, args.temp, args.sigma)
    ...
    logging.info('Private tokens generated - Mean: {:.4f}, Std: {:.4f}'.format(np.mean(results['prv_len']), np.std(results['prv_len'])))
