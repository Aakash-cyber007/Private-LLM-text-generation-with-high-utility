...
if __name__=='__main__':
    ...
    parser.add_argument('--sigma', default=0.01, type=float, help='stddev for noisy screening')
    parser.add_argument('--alpha', default=15, type=float, help='alpha for noisy screening')
    parser.add_argument('--beta', default=0.3, type=float, help='beta for noisy screening')
    parser.add_argument('--lamb', default=1e-4, type=float, help='lambda for probability mixing')
    parser.add_argument('--aor_adj', action='store_true', help='Set true for add-or-remove adjacency; false for replace-by-null adjacency')
    ...
    expt_name = args.model + '-eps_{}-batch_{}-tokens_{}-beta_{}-sigma_{}-lamb_{}-temp_{:.1f}'
    expt_folder = os.path.join(os.path.join(args.folder, args.dataset), args.method)
    expt_name = expt_name.format(args.eps, args.batch, args.tokens,  args.beta, args.sigma, args.lamb, args.temp)
    ...
    for i in tqdm(range(args.num)):
        ...
        for _ in range(args.tokens):
            ...
            # get pub/prv logits clip and get probabilities
            pub_logits, prv_logits = logits[-1], logits[:-1]
            pub_probs = spl.softmax(pub_logits/args.temp)
            prv_probs = spl.softmax(prv_logits/args.temp, axis=1) 
            
            # mix probabilities and carry out noisy screening
            prv_mix = utils.prob_mix(pub_probs, prv_probs, args.lamb)
            pubs, prvs = utils.noisy_screen(pub_probs, prv_mix, topk=args.topk, sigma=args.sigma)
            eps_counter += (args.lamb/((args.batch-1)*args.sigma))**2*args.alpha
            if utils.renyi_div(pubs, prvs) > args.thresh:
                probs = pub_probs
            else:
                lambs, prv_fin = np.zeros(args.batch-1), []
                for ii in range(args.batch-1):
                    def minimize_fn(x):
                        pmix = utils.prob_mix(pub_probs, prv_probs[ii], x)
                        rdiv = utils.renyi_div(pub_probs, pmix, args.alpha)
                        return rdiv - args.beta*args.alpha
                    if minimize_fn(1e-5)*minimize_fn(1-1e-5)>0 or np.isnan(minimize_fn(1e-5)) or np.isnan(minimize_fn(1-1e-5)): lambs[ii]=0
                    else: lambs[ii] = brentq(minimize_fn, 1e-5, 1-1e-5, maxiter=100)
                    prv_fin.append(utils.prob_mix(pub_probs, prv_probs[ii], lambs[ii]))
                prv_fin = np.array(prv_fin)
                probs = np.mean(prv_fin, axis=0)

                divs = []
                ind = np.array(range(len(prv_fin)))
                
                # implement accounting for replace-by-null adjacency and add-or-remove adjacency
                for ii in range(len(prv_fin)):
                    ind2 = np.delete(ind, ii)
                    if args.aor_adj: prv_del = np.mean(prv_fin[ind2], axis=0)
                    else: prv_del = np.sum(prv_fin[ind2], axis=0)/len(prv_fin) + pub_probs/len(prv_fin)
                    divs.append(utils.renyi_div(prv_del, probs, args.alpha))
                eps_counter += max(divs)
            
            
            # check if privacy budget is exceeded
            eps_adp = utils.rdp_eps(eps_counter, args.alpha, args.delta)
            if eps_adp > args.eps: break    
            
            # if privacy budget is not exceeded, sample next token
        ...
        results['eps'].append(min(eps_adp, args.eps))
